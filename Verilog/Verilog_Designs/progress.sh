...
check_subpart() {
    ...
    # Format numbers to look neat
    local old_str="$old_count/$old_expected"
    local new_str="$new_count/$new_expected"

    echo -e "  printf_placeholder" | awk -v n="$name" -v od="$old_done" -v os="$old_str" -v nd="$new_done" -v ns="$new_str" -v sd="$sub_done" 'BEGIN {}' >/dev/null 2>&1
    
    # Pure Bash alignment using spaces
    printf "  %-10s | %s Old: %11s | %s New: %11s | %s\n" \
        "$name" "$old_done" "$old_str" "$new_done" "$new_str" "$sub_done"
...
if [[ "$part4_old_count" -eq "$PART4_OLD" ]] && [[ "$part4_new_count" -eq "$PART4_NEW" ]]; then
    part4_status="${GREEN}DONE${NC}"
else
    part4_status="${YELLOW}PENDING${NC}"
fi

old_str_4="$part4_old_count/$PART4_OLD"
new_str_4="$part4_new_count/$PART4_NEW"
printf "  %-10s | %s Old: %11s | %s New: %11s | %s\n" \
    "Part4" "$part4_old_done" "$old_str_4" "$part4_new_done" "$new_str_4" "$part4_status"
...
PART6_OLD_FILE="./Part6/dataset_power_part6.csv"
PART6_NEW_FILE="./Part6/dataset_power_alt_part6.csv"

part6_old_count=$(check_file "$PART6_OLD_FILE")
part6_new_count=$(check_file "$PART6_NEW_FILE")

if [[ "$part6_old_count" -eq "$PART6_OLD" ]]; then
    part6_old_done="${GREEN}✓${NC}"
else
    part6_old_done="${RED}✗${NC}"
fi

if [[ "$part6_new_count" -eq "$PART6_NEW" ]]; then
    part6_new_done="${GREEN}✓${NC}"
else
    part6_new_done="${RED}✗${NC}"
fi

if [[ "$part6_old_count" -eq "$PART6_OLD" ]] && [[ "$part6_new_count" -eq "$PART6_NEW" ]]; then
    part6_status="${GREEN}DONE${NC}"
else
    part6_status="${YELLOW}PENDING${NC}"
fi

old_str_6="$part6_old_count/$PART6_OLD"
new_str_6="$part6_new_count/$PART6_NEW"
printf "  %-10s | %s Old: %11s | %s New: %11s | %s\n" \
    "Part6" "$part6_old_done" "$old_str_6" "$part6_new_done" "$new_str_6" "$part6_status"

if [[ "$part6_old_count" -eq "$PART6_OLD" ]] && [[ "$part6_new_count" -eq "$PART6_NEW" ]]; then
    echo -e "${GREEN}Part6 COMPLETE!${NC} (Old: ${PART6_OLD} rows, New: ${PART6_NEW} rows)"
else
    echo -e "${YELLOW}Part6 IN PROGRESS${NC} (Old: ${PART6_OLD} rows, New: ${PART6_NEW} rows total)"
fi
...
OLD_TOTAL=$((PART1_OLD_TOTAL + PART2_OLD_TOTAL + PART3_OLD_TOTAL + PART4_OLD + PART5_OLD_TOTAL + PART6_OLD))
NEW_TOTAL=$((PART1_NEW_TOTAL + PART2_NEW_TOTAL + PART3_NEW_TOTAL + PART4_NEW + PART5_NEW_TOTAL + PART6_NEW))
...
for sub in Part1_1 Part1_2 Part1_3; do
    actual_old_total=$((actual_old_total + $(check_file "./Part1/${sub}/dataset_power_${sub}.csv")))
    actual_new_total=$((actual_new_total + $(check_file "./Part1/${sub}/dataset_power_alt_${sub}.csv")))
done

# Part2 summation
for sub in Part2_1 Part2_2 Part2_3; do
    actual_old_total=$((actual_old_total + $(check_file "./Part2/${sub}/dataset_power_${sub}.csv")))
    actual_new_total=$((actual_new_total + $(check_file "./Part2/${sub}/dataset_power_alt_${sub}.csv")))
done

# Part3 summation
for sub in Part3_1 Part3_2 Part3_3; do
    actual_old_total=$((actual_old_total + $(check_file "./Part3/${sub}/dataset_power_${sub}.csv")))
    actual_new_total=$((actual_new_total + $(check_file "./Part3/${sub}/dataset_power_alt_${sub}.csv")))
done

# Part4 summation
actual_old_total=$((actual_old_total + $(check_file "./Part4/dataset_power_part4.csv")))
actual_new_total=$((actual_new_total + $(check_file "./Part4/dataset_power_alt_part4.csv")))

# Part5 summation
for sub in Part5_1 Part5_2 Part5_3; do
    part_number="${sub##*_}"
    actual_old_total=$((actual_old_total + $(check_file "./Part5/${sub}/dataset_power_part5_${part_number}.csv")))
    actual_new_total=$((actual_new_total + $(check_file "./Part5/${sub}/dataset_power_alt_part5_${part_number}.csv")))
done

# Part6 summation
actual_old_total=$((actual_old_total + $(check_file "./Part6/dataset_power_part6.csv")))
actual_new_total=$((actual_new_total + $(check_file "./Part6/dataset_power_alt_part6.csv")))
